(() => {
  ...
  function collectOwnAudioTracks(media2, { excludeSystem = false } = {}) {
    ...
    addTrackOnce(tracks, (_k = (_j = media2 == null ? void 0 : media2.producers) == null ? void 0 : _j.microphone) == null ? void 0 : _k.track);
    addTrackOnce(tracks, (_l = media2 == null ? void 0 : media2.getLocalMicrophoneTrack) == null ? void 0 : _l.call(media2));
    return tracks;
  }
  ...
  function drawElementFixed(ctx, element, canvasWidth, canvasHeight) {
  ...
  // src/shared/whiteboard-engine.js
  var DEFAULT_WIDTH = 1920;
  var DEFAULT_HEIGHT = 1080;
  var DEFAULT_FPS = 30;
  var BACKGROUND = "#ffffff";
  var WhiteboardEngine = {
    start(opts = {}) {
  ...
  var MAX_SLOTS = 4;
  var MIN_RECT_SIZE = 0.05;
  var LAYOUTS = ["full", "split-h", "split-v", "pip-br", "pip-bl"];
  var FULL_FRAME = Object.freeze({ x: 0, y: 0, w: 1, h: 1 });
  function newId() {
  ...
  function normalizeRect(raw, fallback = FULL_FRAME) {
    const base = fallback || FULL_FRAME;
    const x = clamp01((raw == null ? void 0 : raw.x) ?? base.x);
    const y = clamp01((raw == null ? void 0 : raw.y) ?? base.y);
    let w = clamp01((raw == null ? void 0 : raw.w) ?? base.w);
    let h = clamp01((raw == null ? void 0 : raw.h) ?? base.h);
  ...
  function createStudioState({ onChange } = {}) {
    ...
    function updateSlotTransform(sceneId, peerId, { crop, frame, frameEdited } = {}) {
      ...
      if (!slot) return false;
      if (crop !== void 0) slot.crop = normalizeRect(crop, FULL_FRAME);
      if (frame !== void 0) {
        slot.frame = normalizeRect(frame, FULL_FRAME);
        if (frameEdited !== void 0) slot.frameEdited = !!frameEdited;
  ...
  function defaultFramesForLayout(layout, slotCount) {
  ...
  function createHiddenVideo(stream, ownerDocument = document) {
    ...
    video.playsInline = true;
    video.autoplay = true;
    video.style.cssText = "position:fixed;left:-9999px;top:0;width:1920px;height:1080px;opacity:0;pointer-events:none";
  ...
  function resolveFrameForSlot(slot, layout, slotCount, index) {
    if ((slot == null ? void 0 : slot.frameEdited) && (slot == null ? void 0 : slot.frame)) return slot.frame;
    if ((slot == null ? void 0 : slot.frame) && (slotCount === 1 || slot.frameEdited)) return slot.frame;
    const defaults = defaultFramesForLayout(layout, slotCount);
    return defaults[index] || (slot == null ? void 0 : slot.frame) || FULL_FRAME;
  }
  ...
  // src/shared/studio-transform-editor.js
  var HANDLES = ["nw", "n", "ne", "e", "se", "s", "sw", "w"];
  function createStudioTransformEditor({
    container,
    onChange,
    getScene,
    getSelectedPeerId,
    ...
    function debouncedChange(sceneId, peerId, patch) {
      ...
      debounceTimer = setTimeout(() => {
        debounceTimer = null;
        onChange == null ? void 0 : onChange(sceneId, peerId, patch);
      }, 50);
    }
    function getContainerRect() {
      return container.getBoundingClientRect();
    }
    function clientToNormalized2(clientX, clientY) {
    ...
    function refresh() {
      ...
      container.innerHTML = "";
      const selectedPeerId = getSelectedPeerId == null ? void 0 : getSelectedPeerId();
      const cropMode = !!(getCropMode == null ? void 0 : getCropMode());
      scene.slots.forEach((slot, index) => {
    ...
    function applyDrag(clientX, clientY) {
      ...
      const delta = clientToNormalized2(clientX, clientY);
      const startNorm = clientToNormalized2(dragState.startClient.x, dragState.startClient.y);
    ...
    container.addEventListener("pointerdown", (e) => {
      if (e.target === container && e.button === 0) {
        setSelectedPeerId == null ? void 0 : setSelectedPeerId(null);
        refresh();
      }
    });
    function dispose() {
  ...
  // src/host/app.js
  var STUDIO_COMPOSITOR_IN_MAIN = true;
  ...
  var studioTransformEditor = null;
  var studioCropMode = false;
  var studioPreviewApplyTimer = null;
  var studioLastPreviewKey = "";
  var studioProgramCanvasHost = null;
  var hostVideoWatchdogId = null;
  var whiteboardEngine = null;
  var whiteboardActiveLocal = false;
  var lastWhiteboardServerElements = [];
  var lastTransmissionSourceKind = null;
    ...
    try {
      const res = await fetch("/api/audio-filter", {
        ...
        body: JSON.stringify({
          ...
          userId: userId || (authUser == null ? void 0 : authUser.id) || null
        })
      });
  ...
  async function applyHostMicPublishGain(value) {
    ...
    if (hostDisplayName) {
      saveAudioFiltersPresetDebounced(hostDisplayName, prefs, "host", authUser == null ? void 0 : authUser.id);
    }
  }
  async function loadHostMicPresetFromStorage() {
    ...
    let prefs = null;
    if (hostDisplayName) {
      const apiPreset = await fetchAudioFilterPresetApi("host", hostDisplayName, authUser == null ? void 0 : authUser.id);
    ...
    if (!prefs) {
      ...
      if (hostDisplayName && hasActiveMicrophoneFilter(prefs)) {
        saveAudioFilterPresetApi("host", hostDisplayName, prefs, authUser == null ? void 0 : authUser.id).catch(() => {
        });
      }
    }
  ...
  function promptRoomPinIfRequired(roomPinRequired) {
    ...
    return new Promise((resolve) => {
      ...
      (_c = els.pinInput) == null ? void 0 : _c.focus();
    });
  }
  function promptHostScreenShare() {
    return new Promise((resolve) => {
      ...
      if (desc) {
        desc.textContent = `Ol\xE1, ${hostDisplayName}. Selecione a tela que deseja transmitir neste painel.`;
      }
      ...
      els.btnHostEntrySubmit.onclick = submit;
      (_c = els.hostNameInput) == null ? void 0 : _c.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
          e.preventDefault();
          submit();
        }
      });
      (_d = els.btnHostEntrySubmit) == null ? void 0 : _d.focus();
    });
  }
  var roomPin = "";
  var hostToken = "";
  ...
  function debugPopoutLog(hypothesisId, location2, message, data = {}) {
    fetch("/api/client-debug", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function onHostAudioPrefsChange() {
    ...
    try {
      ...
      if (prefs.microphone && !result.ok && result.reason !== "disabled") {
        ...
        showToast2("Microfone desativado - so audio da aba/janela", "info");
  ...
  function updatePreviewOverlays() {
    ...
    const badgeName = isWhiteboardTransmission(lastActiveTransmission) ? "Quadro branco" : sel == null ? void 0 : sel.displayName;
  ...
  function applyWhiteboardState(payload) {
    ...
    whiteboardEngine == null ? void 0 : whiteboardEngine.setElements(elements);
    whiteboardActiveLocal = !!payload.active;
    updateQuadroBrancoUi();
  }
  function handleWhiteboardElement(element) {
    whiteboardEngine == null ? void 0 : whiteboardEngine.addElement(element);
  }
  async function ensureWhiteboardEngineReady(elements = lastWhiteboardServerElements) {
    if (whiteboardEngine == null ? void 0 : whiteboardEngine.stream) {
  ...
  async function iniciarQuadroBranco() {
    ...
    try {
      ...
      updateQuadroBrancoUi();
      showToast2("Quadro branco ativo", "success");
  ...
  function updateDrawUi() {
    const hasPreview = ui._flags.hasPreview;
    annotationToolbar == null ? void 0 : annotationToolbar.setVisible(hostReady && hasPreview);
    annotationToolbar == null ? void 0 : annotationToolbar.syncClearVisibility();
  ...
  function getHostVideoStreamForStudio() {
    ...
    if (media == null ? void 0 : media._syntheticStream) {
      const synTrack = (_g = (_f = (_e = media._syntheticStream).getVideoTracks) == null ? void 0 : _f.call(_e)) == null ? void 0 : _g[0];
      if ((synTrack == null ? void 0 : synTrack.readyState) === "live") return media._syntheticStream;
    }
    if (whiteboardEngine == null ? void 0 : whiteboardEngine.stream) {
      const wbTrack = (_j = (_i = (_h = whiteboardEngine.stream).getVideoTracks) == null ? void 0 : _i.call(_h)) == null ? void 0 : _j[0];
      if ((wbTrack == null ? void 0 : wbTrack.readyState) === "live") return whiteboardEngine.stream;
    }
    if (studioProgramCompositor == null ? void 0 : studioProgramCompositor.stream) {
      const compTrack = (_m = (_l = (_k = studioProgramCompositor.stream).getVideoTracks) == null ? void 0 : _l.call(_k)) == null ? void 0 : _m[0];
      if ((compTrack == null ? void 0 : compTrack.readyState) === "live") return studioProgramCompositor.stream;
    }
    return null;
  }
  ...
  function sceneNeedsCompositor(scene, slots = null) {
    const list = slots || (scene == null ? void 0 : scene.slots) || [];
    if (list.length > 1) return true;
    return list.some((slot) => slotNeedsTransform(slot));
  }
  function getStudioPreviewKey(scene) {
    if (!scene) return "";
    return scene.slots.map((s) => `${s.peerId}:${s.producerId}:${JSON.stringify(s.crop)}:${JSON.stringify(s.frame)}:${s.frameEdited}`).join("|");
  }
  function ensureStudioProgramCanvasHost() {
    if (studioProgramCanvasHost == null ? void 0 : studioProgramCanvasHost.isConnected) return studioProgramCanvasHost;
    let el = document.getElementById("studio-program-compositor-canvas");
    if (!el) {
      el = document.createElement("canvas");
      el.id = "studio-program-compositor-canvas";
      el.style.cssText = "position:fixed;left:-9999px;top:0;width:1920px;height:1080px;opacity:0;pointer-events:none";
      document.body.appendChild(el);
    }
  ...
  function disposeStudioProgramCanvasHost() {
    if (studioProgramCanvasHost == null ? void 0 : studioProgramCanvasHost.parentNode) {
  ...
  function startHostVideoWatchdog() {
    ...
    hostVideoWatchdogId = setInterval(async () => {
      ...
      if (trackState === "live") {
        await ((_c = media == null ? void 0 : media.repairHostVideoIfNeeded) == null ? void 0 : _c.call(media).catch(() => {
        }));
        return;
      }
      if (trackState !== "live" && ((_d = media == null ? void 0 : media.producers) == null ? void 0 : _d.video) && !media.producers.video.closed) {
        showToast2("Transmiss\xE3o do host interrompida \u2014 recompartilhe a tela", "warn");
  ...
  async function waitForVideoDimensions(videoEl2, timeoutMs = 2500) {
    if (!videoEl2) return;
    if (videoEl2.videoWidth > 0 && videoEl2.videoHeight > 0) return;
    await new Promise((resolve) => {
      const timer = setTimeout(resolve, timeoutMs);
      const done = () => {
        clearTimeout(timer);
        videoEl2.removeEventListener("loadeddata", done);
        videoEl2.removeEventListener("resize", done);
        resolve();
      };
      videoEl2.addEventListener("loadeddata", done);
      videoEl2.addEventListener("resize", done);
    });
  }
  ...
  function buildStudioParticipantCard(c, onSelect) {
    ...
    const sceneSlotPeerIds = new Set(((editing == null ? void 0 : editing.slots) || []).map((s) => String(s.peerId)));
  ...
  function buildSourceCard(c, onSelect, isTransmissionSection = false, studioOptions = null) {
    const ownerDocument = (studioOptions == null ? void 0 : studioOptions.ownerDocument) || getListaOwnerDocument();
    const card = buildDisplaySourceCard(c, onSelect, {
      forStudioSlot: studioOptions == null ? void 0 : studioOptions.forStudioSlot,
      sceneSlotPeerIds: studioOptions == null ? void 0 : studioOptions.sceneSlotPeerIds,
      noSharingHighlight: (studioOptions == null ? void 0 : studioOptions.forStudioSlot) || !isTransmissionSection,
      ownerDocument,
  ...
  function renderLista() {
    ...
    debugPopoutLog("A,B,E", "host/app.js:renderLista", "start", {
      ...
      sidebarHidden: !!(sidebar == null ? void 0 : sidebar.hidden),
      listaDocIsMain: listDoc === document,
      listaConnected: els.lista.isConnected,
      listaParentId: ((_b = els.lista.parentElement) == null ? void 0 : _b.id) || null
    });
    els.lista.innerHTML = "";
    ...
    debugClientSessionLog("H2", "host:renderLista", "render participants", {
      totalClients: ((_c = estado.clients) == null ? void 0 : _c.length) ?? 0,
      remoteCount: participants.length,
      remoteNames: participants.map((c) => c.displayName),
      listaConnected: els.lista.isConnected,
      sectionH: ((_d = els.lista.closest(".participants-section")) == null ? void 0 : _d.offsetHeight) ?? 0,
      listaH: els.lista.offsetHeight,
      hypothesisId: "C"
    });
    debug3a36beLog("C", "host:renderLista", "render participants", {
      totalClients: ((_e = estado.clients) == null ? void 0 : _e.length) ?? 0,
      participantCount: participants.length,
      participantNames: participants.map((c) => c.displayName),
      hasListaEl: !!els.lista,
      listaConnected: ((_f = els.lista) == null ? void 0 : _f.isConnected) ?? false,
      participantsSectionFound: !!((_g = els.lista) == null ? void 0 : _g.closest(".participants-section")),
      sectionH: ((_i = (_h = els.lista) == null ? void 0 : _h.closest(".participants-section")) == null ? void 0 : _i.offsetHeight) ?? 0,
      listaH: ((_j = els.lista) == null ? void 0 : _j.offsetHeight) ?? 0,
      sidebarCollapsed: ((_k = els.sidebar) == null ? void 0 : _k.classList.contains("is-collapsed")) ?? false
    });
    ...
    for (const c of sortClientsForDisplay(participants)) {
      const card = shouldUseStudioParticipantCards() ? buildStudioParticipantCard(c, onSelect) : buildSourceCard(c, onSelect, false);
      cardKinds.push({
        name: c.displayName,
        kind: card.classList.contains("sharing") ? "sharing" : card.classList.contains("available") ? "available" : card.classList.contains("spectator") ? "spectator" : "other"
      });
    ...
    ui.set({
      ...
      isPaused: !!((_l = estado.selecionado) == null ? void 0 : _l.pausado)
    });
    updatePreviewOverlays();
    ...
    requestAnimationFrame(() => {
      ...
      debugPopoutLog("F", "host/app.js:renderLista", "layout", {
        listaOffsetHeight: ((_c2 = els.lista) == null ? void 0 : _c2.offsetHeight) ?? 0,
        listaClientHeight: ((_d2 = els.lista) == null ? void 0 : _d2.clientHeight) ?? 0,
        listaRectH: listaRect ? Math.round(listaRect.height) : 0,
        sectionOffsetHeight: (participantsSection == null ? void 0 : participantsSection.offsetHeight) ?? 0,
      ...
      if (!tx.producerId) {
        ...
        if (isWhiteboardTransmission(tx)) {
          if (whiteboardEngine == null ? void 0 : whiteboardEngine.stream) {
            ...
            previewStream = (engine == null ? void 0 : engine.stream) || null;
          }
        } else if (((_e = media == null ? void 0 : media.isSyntheticVideoActive) == null ? void 0 : _e.call(media)) && (studioProgramCompositor == null ? void 0 : studioProgramCompositor.stream)) {
        ...
        updatePreviewOverlays();
        const statusMsg = !previewBound ? "Preview indisponivel" : isWhiteboardTransmission(tx) ? "Exibindo quadro branco" : ((_g = media == null ? void 0 : media.isSyntheticVideoActive) == null ? void 0 : _g.call(media)) ? "Exibindo cena composta" : "Exibindo sua tela";
        setStatus(statusMsg);
      } else {
        if (gen !== transmissionGeneration) return;
        const ownProducerId = ((_i = (_h = media.producers) == null ? void 0 : _h.video) == null ? void 0 : _i.id) || null;
        const currentProducerId = media.currentActiveVideoProducerId || ((_k = (_j = media.remoteConsumers) == null ? void 0 : _j.video) == null ? void 0 : _k.producerId);
        const needsConsume = remoteVideoConsumeNeeded(tx, {
          currentProducerId,
          isSelfSelected: false,
          hasVideoElement: !!((_l = els.preview) == null ? void 0 : _l.srcObject),
          consumerClosed: !((_m = media == null ? void 0 : media.remoteConsumers) == null ? void 0 : _m.video) || media.remoteConsumers.video.closed
        });
        if (needsConsume) {
        ...
        if (!tx.paused) {
          try {
            await ((_q = (_p = els.preview) == null ? void 0 : _p.play) == null ? void 0 : _q.call(_p));
          } catch (_) {
          }
        }
      }
    } catch (e) {
      if (((_r = e.message) == null ? void 0 : _r.includes("Autoplay")) || e.name === "NotAllowedError") {
        hostMicAutoplayNeeded = true;
        updateHostMicUi();
      }
  ...
  function resolveTransmissionForEnrich(snapshot, parsed) {
    const snapTx = parsed == null ? void 0 : parsed.transmission;
  ...
  function isHostPeer(c) {
    return !!((c == null ? void 0 : c.ehHost) || (c == null ? void 0 : c.role) === "host" || hostPeerId && String(c == null ? void 0 : c.id) === String(hostPeerId));
  }
  ...
  function applyParticipantState(snapshot, { source = "unknown" } = {}) {
    ...
    debug3a36beLog("A", "host:applyParticipantState", "participants applied", {
      ...
      clientNames: roomClients.map((c) => c.displayName),
      remoteCount: roomClients.filter((c) => !isHostPeer(c)).length,
      hostPeerId: (hostPeerId == null ? void 0 : hostPeerId.slice(0, 8)) || null
    });
  ...
  async function requestRoomStateSync() {
    if (!(signaling == null ? void 0 : signaling.connected)) return;
    try {
      const snapshotPromise = signaling.onceType("roomState", () => true, 5e3);
  ...
  function handleMessage(msg) {
    ...
    if (msg.type === "quadroBrancoLimpar") {
      lastWhiteboardServerElements = [];
      whiteboardEngine == null ? void 0 : whiteboardEngine.clear();
      drawingSurface == null ? void 0 : drawingSurface.clearPersistentOverlay();
      return;
    }
    if (msg.type === "estado") {
    ...
    if (msg.type === "audioPolicyAplicada") {
      if (media) {
        media.applyAudioPolicyFromServer(msg.payload || {}).catch(
...
${entry.technical}`;
  ...
  async function bootstrap() {
    ...
    if (cohostParam && tokenParam) {
      ...
      try {
        ...
        showToast2(e.message || "Falha na autentica\xE7\xE3o", "error");
        return;
      }
      try {
  ...
  var sidebarPopoutTransition = false;
  var sidebarWasCollapsed = false;
  var sidebarHiddenForPopout = false;
  var studioPopoutRoot = null;
  var popoutControlsExpanded = false;
  var POPOUT_WIDTH_BASIC = 286;
  var POPOUT_WIDTH_EXPANDED = 960;
  var POPOUT_EXPAND_ICON = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>';
  ...
  function syncPopoutLayoutExpanded(expanded) {
    ...
    try {
      ...
      layout == null ? void 0 : layout.classList.toggle("is-expanded", expanded);
      layout == null ? void 0 : layout.classList.toggle("is-basic", !expanded);
  ...
  function setPopoutControlsExpanded(expanded) {
    ...
    if (studioUi == null ? void 0 : studioUi.root) {
      ...
      if (studioUi.btnExpandControls) {
        studioUi.btnExpandControls.textContent = popoutControlsExpanded ? "Ocultar controles avan\xE7ados" : "Expandir controles";
        studioUi.btnExpandControls.setAttribute("aria-expanded", String(popoutControlsExpanded));
      }
      if (studioUi.popoutModeLabel) {
        studioUi.popoutModeLabel.textContent = popoutControlsExpanded ? "Expandido" : "Padr\xE3o";
      }
    }
    syncPopoutExpandButtonInSidebar();
    if (controlsPopoutWindow && !controlsPopoutWindow.closed) {
      try {
        controlsPopoutWindow.document.title = popoutControlsExpanded && studio.isStudioModeEnabled() ? "Studio \u2014 ShareScreen" : "Controles \u2014 ShareScreen";
      } catch (_) {
      }
    }
  ...
  async function buildStudioPreviewSources(scene) {
    ...
    for (const slot of scene.slots) {
      ...
      if (hostPeerId && String(slot.peerId) === String(hostPeerId)) {
        ...
        const track = (_d = (_c = hostStream == null ? void 0 : hostStream.getVideoTracks) == null ? void 0 : _c.call(hostStream)) == null ? void 0 : _d[0];
  ...
  async function applyStudioPreview(sceneId) {
    ...
    if (previewKey === studioLastPreviewKey && studioPreviewCompositor) {
      (_b = studioTransformEditor == null ? void 0 : studioTransformEditor.refresh) == null ? void 0 : _b.call(studioTransformEditor);
      return;
    }
    ...
    if (!validCount) {
      ...
      if (studioUi.transformOverlay) studioUi.transformOverlay.hidden = true;
      if (studioUi.btnTransition) studioUi.btnTransition.disabled = true;
      (_c = studioTransformEditor == null ? void 0 : studioTransformEditor.refresh) == null ? void 0 : _c.call(studioTransformEditor);
      return;
    }
    const dims = getStudioCompositorSize(sources);
    const useCompositor = sceneNeedsCompositor(scene);
    if (!useCompositor && scene.slots.length === 1 && ((_d = sources[0]) == null ? void 0 : _d.videoEl)) {
      ...
      if (studioUi.previewVideo && stream) {
        ...
        (_f = (_e = studioUi.previewVideo).play) == null ? void 0 : _f.call(_e).catch(() => {
        });
        if (studioUi.btnTransition) studioUi.btnTransition.disabled = false;
        (_g = studioTransformEditor == null ? void 0 : studioTransformEditor.refresh) == null ? void 0 : _g.call(studioTransformEditor);
        return;
      }
      if ((_h = sources[0]) == null ? void 0 : _h.stream) {
        if (studioUi.previewVideo) {
          ...
          studioUi.previewEmpty.hidden = true;
          if (studioUi.transformOverlay) studioUi.transformOverlay.hidden = false;
          (_j = (_i = studioUi.previewVideo).play) == null ? void 0 : _j.call(_i).catch(() => {
          });
          if (studioUi.btnTransition) studioUi.btnTransition.disabled = false;
          (_k = studioTransformEditor == null ? void 0 : studioTransformEditor.refresh) == null ? void 0 : _k.call(studioTransformEditor);
          return;
        }
      }
    }
    ...
    (_l = studioTransformEditor == null ? void 0 : studioTransformEditor.refresh) == null ? void 0 : _l.call(studioTransformEditor);
  }
  ...
  async function ensureSceneCompositor(scene, validSlots, { forProgram = false } = {}) {
    const snap = studio.getSnapshot();
    if (!forProgram && (studioPreviewCompositor == null ? void 0 : studioPreviewCompositor.stream) && snap.previewSceneId === scene.id && validSlots.length >= 1) {
    ...
    const dims = getStudioCompositorSize(sources);
    const canvas = forProgram && STUDIO_COMPOSITOR_IN_MAIN ? ensureStudioProgramCanvasHost() : void 0;
    const ownerDocument = forProgram && STUDIO_COMPOSITOR_IN_MAIN ? document : void 0;
  ...
  function handleStudioAddParticipant(peerId) {
    ...
    if (!(client == null ? void 0 : client.id)) return;
  ...
  function syncStudioCropModeUi() {
    if (studioUi == null ? void 0 : studioUi.btnCropMode) {
  ...
  function mountStudioPopoutShell(win) {
    ...
    win.document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Escape") {
        ...
        (_c2 = studioUi == null ? void 0 : studioUi.btnResetSlot) == null ? void 0 : _c2.click();
      } else if (e.altKey && !e.ctrlKey && !e.metaKey) {
        studioCropMode = true;
        syncStudioCropModeUi();
        (_d2 = studioTransformEditor == null ? void 0 : studioTransformEditor.refresh) == null ? void 0 : _d2.call(studioTransformEditor);
      }
    });
    ...
    (_d = studioUi == null ? void 0 : studioUi.modeToggle) == null ? void 0 : _d.addEventListener("change", () => {
    ...
    (_e = studioUi == null ? void 0 : studioUi.btnTransition) == null ? void 0 : _e.addEventListener("click", () => executeStudioTransition());
    (_f = studioUi == null ? void 0 : studioUi.btnNew) == null ? void 0 : _f.addEventListener("click", () => {
      studio.createScene({ name: `Cena ${studio.getSnapshot().scenes.length + 1}` });
      refreshStudioUi();
    });
    (_g = studioUi == null ? void 0 : studioUi.btnDup) == null ? void 0 : _g.addEventListener("click", () => {
      const editing = studio.getEditingScene();
      if (editing) studio.duplicateScene(editing.id);
      refreshStudioUi();
    });
    (_h = studioUi == null ? void 0 : studioUi.btnDel) == null ? void 0 : _h.addEventListener("click", () => {
      const editing = studio.getEditingScene();
      if (!editing) return;
    ...
    (_i = studioUi == null ? void 0 : studioUi.sceneName) == null ? void 0 : _i.addEventListener("change", () => {
      const editing = studio.getEditingScene();
      if (editing) {
  ...
  function teardownStudioPopout() {
    ...
    stopStudioPreviewCompositor();
    stopStudioProgramCompositor();
    (_b = media == null ? void 0 : media.closePreviewConsumers) == null ? void 0 : _b.call(media).catch(() => {
    });
    studioUi = null;
  ...
  function ensurePopoutExpandButtonInSidebar() {
    const sidebar = els.sidebar;
    const actions = sidebar == null ? void 0 : sidebar.querySelector(".sidebar-actions");
    if (!actions) return;
    ...
    if (!btn) {
      ...
      btn.setAttribute("aria-label", "Expandir controles avan\xE7ados");
      btn.title = "Expandir controles avan\xE7ados";
  ...
  function moveSidebarToPopout(win) {
    ...
    if (!placeholder) {
      ...
      (_b = sidebar.parentNode) == null ? void 0 : _b.insertBefore(placeholder, sidebar);
    }
  ...
  function openControlsPopout() {
    ...
    const features = `width=${POPOUT_WIDTH_BASIC},height=720,menubar=no,toolbar=no,location=no,status=no,resizable=yes,scrollbars=yes`;
    ...
    try {
      if (!win.document.getElementById("popout-layout") || !moveSidebarToPopout(win)) {
        ...
        teardownStudioPopout();
        win.close();
        showToast2("Falha ao abrir painel de controles", "error");
        return;
      }
      ...
      debugPopoutLog("C,D", "host/app.js:openControlsPopout", "popout opened", {
        popoutLayoutChildren: ((_c = win.document.getElementById("popout-layout")) == null ? void 0 : _c.childElementCount) ?? 0,
        sidebarInPopout: ((_d = els.sidebar) == null ? void 0 : _d.ownerDocument) === win.document,
        popoutBodyChildCount: ((_e = win.document.body) == null ? void 0 : _e.childElementCount) ?? 0
      });
      if (!studioProgramMirrorId) {
        studioProgramMirrorId = setInterval(syncStudioProgramMirror, 500);
      }
  ...
  annotationToolbar = createAnnotationToolbar({
    ...
    onClear: () => {
      ...
      signaling == null ? void 0 : signaling.send("quadroBrancoLimpar");
      whiteboardEngine == null ? void 0 : whiteboardEngine.clear();
      drawingSurface == null ? void 0 : drawingSurface.clearPersistentOverlay();
    }
  });
    ...
    for (const source of audioSources || []) {
      ...
      if (displayName) {
        ...
        if (apiPreset == null ? void 0 : apiPreset.prefs) {
          ...
          saveAudioFilterPresetApi("client", displayName, stored, (client == null ? void 0 : client.userId) || null).catch(() => {
          });
        }
      }

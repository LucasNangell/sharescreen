...
* **Filtros de audio por client:** `src/host/app.js` envia preferencias salvas, `src/client/app.js` recebe e `src/shared/media-client.js` aplica o filtro de microfone antes do producer WebRTC; `src/shared/host-audio-monitor.js` mantem o monitor local do host; `src/shared/mic-dsp.js` centraliza defaults e cadeia DSP de publicacao/monitor.
...
4. Opcionalmente inicializa a gravação local, enviando os chunks do `MediaRecorder` em background.

...
* **Filtros de audio por client:** `src/host/app.js` envia preferencias salvas (API `/api/audio-filter` + cache localStorage), `src/client/app.js` recebe e `src/shared/media-client.js` aplica o filtro de microfone antes do producer WebRTC; `mic-dsp.js` centraliza defaults, portão de proximidade (`micSensitivity` + `micCaptureDistance`) e cadeia DSP. **Os filtros são aplicados apenas na origem**, então todos os participantes recebem a mesma trilha filtrada; `src/shared/host-audio-monitor.js` reproduz o áudio remoto em passthrough (só volume/mute por canal, `playbackDsp` desligado por padrão) para o host ouvir exatamente o que os clients ouvem.

...
8. `anotacaoSegmento` (qualquer peer autenticado) valida payload (pontos normalizados, `shape` opcional `stroke`|`rect`, rate-limit ~20/s) e rebroadcast via `room.broadcastToRoom()` como `anotacaoSegmento` para todos exceto o remetente.
